def insert_user_general_questions(cursor, profile_data):
    cursor.execute(
        """
        SELECT COUNT(*) FROM User_profile_general_questions WHERE user_id = ?
        """,
    ...
    if exists:
        cursor.execute(
            """
            UPDATE User_profile_general_questions SET
                g_q1 = ?, g_q2 = ?, g_q3 = ?, g_q4 = ?, g_q5 = ?, 
                g_q6 = ?, g_q7 = ?, g_q8 = ?, g_q9 = ?, g_q10 = ?
            WHERE user_id = ?
            """,
            (
                profile_data["g_q1"],
